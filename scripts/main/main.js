          ...
          if (button.classList.contains("btn-operator")) {
            switch (buttonSymbol) {
              ...
              case "+/-":
                ...
                break;
              case "x^2":
                  this.display.innerText = this.display.textContent * this.display.textContent ;
                break;
              // Agrega más casos según sea necesario
              default:
                  if(this.hasOperator()) {
                        // Reemplaza el operador existente si ya hay uno
                        this.tempDisplay.innerText = this.tempDisplay.innerText.slice(0, -1) + buttonSymbol;
                      }else {
                        // Añade el operador al tempDisplay y mantiene el número en el display
                        this.tempDisplay.innerText = this.display.innerText + buttonSymbol;
                      }
            }
